...
async function copyTemplateAndTweak(templateName, targetDir, replacements) {
...
async function installDependencies(target, pm) {
    const pkgManager = pm ?? 'npm';
    const installCmd = pkgManager === 'pnpm' ? 'pnpm install' : pkgManager === 'yarn' ? 'yarn' : pkgManager === 'bun' ? 'bun install' : 'npm install';
    await exec(installCmd, { cwd: target });
}
async function scaffoldCursor(root) {
    const cursorDir = path.join(root, '.cursor');
    await ensureDir(cursorDir);
    await writeFile(path.join(cursorDir, 'context.json'), JSON.stringify({ project: { name: path.basename(root) }, createdAt: new Date().toISOString() }, null, 2));
    await ensureDir(path.join(cursorDir, 'rules'));
}
async function scaffoldReadme(root, appName) {
